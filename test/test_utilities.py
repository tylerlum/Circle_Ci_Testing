...
class TestUtilities(unittest.TestCase):
    ...
    def test_getDesiredHeading_north(self):
        ...
        # Setup destination
        northDestination = distance(
            kilometers=1).destination(
            point=(
                position.lat,
                position.lon),
    ...
    def test_globalWaypointReached(self):
        ...
        unreachedPosition = distance(
            kilometers=2 *
            GLOBAL_WAYPOINT_REACHED_RADIUS_KM).destination(
            point=(
                position.lat,
                position.lon),
            bearing=BEARING_EAST)
        ...
        reachedPosition = distance(
            kilometers=GLOBAL_WAYPOINT_REACHED_RADIUS_KM /
            2).destination(
            point=(
                position.lat,
                position.lon),
    ...
    def test_getLocalWaypointLatlon(self):
        ...
        indexOutOfBoundsLatlon = getLocalWaypointLatLon(
            localPath=localPath, localPathIndex=invalidIndex)
        self.assertAlmostEqual(indexOutOfBoundsLatlon.lat,
                               localPath[len(localPath) - 1].lat, places=1)
        self.assertAlmostEqual(indexOutOfBoundsLatlon.lon,
                               localPath[len(localPath) - 1].lon, places=1)

        # Test index in bounds. Should give back correct latlon
        validIndex = len(localPath) // 2
        indexInBoundsLatlon = getLocalWaypointLatLon(
            localPath=localPath, localPathIndex=validIndex)
...
#        plt.show()
    ...
    def test_headingToBearingDegrees(self):
        ...
        self.assertAlmostEqual(
            BEARING_NORTH %
            360,
            headingToBearingDegrees(HEADING_NORTH) %
            360,
            places=3)
        self.assertAlmostEqual(
            BEARING_SOUTH %
            360,
            headingToBearingDegrees(HEADING_SOUTH) %
            360,
            places=3)
        self.assertAlmostEqual(
            BEARING_EAST %
            360,
            headingToBearingDegrees(HEADING_EAST) %
            360,
            places=3)
        ...
        self.assertAlmostEqual(
            bearingDirection %
            360,
            headingToBearingDegrees(headingDirection) %
            360,
            places=3)
    ...
    def test_measuredWindToGlobalWind_basic(self):
        ...
        globalWindSpeedKmph, globalWindDirectionDegrees = measuredWindToGlobalWind(
            measuredWindSpeed=0, measuredWindDirectionDegrees=BOAT_RIGHT, boatSpeed=1, headingDegrees=HEADING_NORTH)
        ...
        globalWindSpeedKmph, globalWindDirectionDegrees = measuredWindToGlobalWind(
            measuredWindSpeed=1.2, measuredWindDirectionDegrees=measuredDirection, boatSpeed=0, headingDegrees=HEADING_EAST)
        ...
        globalWindSpeedKmph, globalWindDirectionDegrees = measuredWindToGlobalWind(
            measuredWindSpeed=5, measuredWindDirectionDegrees=BOAT_FORWARD, boatSpeed=7, headingDegrees=90)
    ...
    def test_globalWindToMeasuredWind_basic(self):
        ...
        measuredWindSpeedKmph, measuredWindDirectionDegrees = globalWindToMeasuredWind(
            globalWindSpeed=0, globalWindDirectionDegrees=HEADING_EAST, boatSpeed=2, headingDegrees=HEADING_EAST)
        self.assertAlmostEqual(2, measuredWindSpeedKmph, places=3)
        self.assertAlmostEqual(
            (BOAT_FORWARD + 180) %
            360, measuredWindDirectionDegrees %
            360, places=3)
        ...
        measuredWindSpeedKmph, measuredWindDirectionDegrees = globalWindToMeasuredWind(
            globalWindSpeed=10, globalWindDirectionDegrees=globalDirection, boatSpeed=0, headingDegrees=HEADING_EAST)
        ...
        measuredWindSpeedKmph, measuredWindDirectionDegrees = globalWindToMeasuredWind(
            globalWindSpeed=10, globalWindDirectionDegrees=HEADING_SOUTH, boatSpeed=3, headingDegrees=HEADING_SOUTH)
    ...
    def test_globalWindToMeasuredWind_and_measuredWindToGlobalWind(self):
        ...
        measuredWindSpeedKmph, measuredWindDirectionDegrees = globalWindToMeasuredWind(
            globalWindSpeedKmph, globalWindDirectionDegrees, boatSpeedKmph, headingDegrees)
        calculatedGlobalWindSpeedKmph, calculatedGlobalWindDirectionDegrees = measuredWindToGlobalWind(
            measuredWindSpeedKmph, measuredWindDirectionDegrees, boatSpeedKmph, headingDegrees)
        ...
        self.assertAlmostEqual(calculatedGlobalWindDirectionDegrees,
                               globalWindDirectionDegrees, places=3)
    ...
    def test_createLocalPathSS(self):
        ...
        for stateIndex in range(1, len(solutionPath.getStates())):
            ...
            distance = ((currState.getX() - prevState.getX())**2 +
                        (currState.getY() - prevState.getY())**2)**0.5
            self.assertTrue(
    ...
    def test_upwindOrDownwindOnPath(self):
        ...
        # Set state with global wind direction nearly same as boat current
        # direction (sailing downwind)
        ...
        measuredWindSpeedKmph, measuredWindDirectionDegrees = globalWindToMeasuredWind(
            globalWindSpeed=10, globalWindDirectionDegrees=downwindGlobalWindDirectionDegrees, boatSpeed=1, headingDegrees=120)
        ...
        # Set state with global wind direction nearly 180 degrees from boat
        # current direction (sailing upwind)
        nextLocalWaypointIndex = 1
        boatPositionXY = latlonToXY(state.position, referenceLatlon)
        desiredHeadingDegrees = math.degrees(
            math.atan2(
        ...
        measuredWindSpeedKmph, measuredWindDirectionDegrees = globalWindToMeasuredWind(
            globalWindSpeed=10, globalWindDirectionDegrees=upwindGlobalWindDirectionDegrees, boatSpeed=1, headingDegrees=120)
        ...
        measuredWindSpeedKmph, measuredWindDirectionDegrees = globalWindToMeasuredWind(
            globalWindSpeed=10, globalWindDirectionDegrees=validGlobalWindDirectionDegrees, boatSpeed=1, headingDegrees=120)
