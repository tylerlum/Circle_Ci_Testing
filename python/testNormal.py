...
def localWaypointReached(position, localPath, localPathIndex):
    ...
    previousWaypoint = latlon(
        float(localPath[localPathIndex - 1].getY()), float(localPath[localPathIndex - 1].getX()))
    localWaypoint = latlon(
        float(
            localPath[localPathIndex].getY()), float(
            localPath[localPathIndex].getX()))
    isStartNorth = localWaypoint.lat < previousWaypoint.lat
    isStartEast = localWaypoint.lon < previousWaypoint.lon

    tangentSlope = (localWaypoint.lat - previousWaypoint.lat) / \
        (localWaypoint.lon - previousWaypoint.lon)
    ...
    def y(x): return normalSlope * x
    def x(y): return y / float(normalSlope)
