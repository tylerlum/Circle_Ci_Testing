...
class BoatState:
    ...
    def __str__(self):
        return (("Global waypoint: {0}\n"
                 ...
                 "AISData: {4}\n"
                 "Heading degrees: {5}\n"
                 "Speed kmph: {6}\n") .format(self.globalWaypoint,
                                              self.position,
                                              self.measuredWindDirectionDegrees,
                                              self.measuredWindSpeedKmph,
                                              self.AISData.ships,
                                              self.headingDegrees,
                                              self.speedKmph))
...
class Sailbot:
    def getCurrentState(self):
        ...
        return BoatState(self.globalPath[self.globalPathIndex],
                         self.position,
                         self.measuredWindDirectionDegrees,
                         self.measuredWindSpeedKmph,
                         self.AISData,
    ...
    def globalPathCallback(self, data):
        ...
        if self.isNewPath(self.globalPath, data.waypoints):
            ...
            rospy.logwarn_once(
                "New global path is the same as the current path. This warning will only show once")
    ...
    def isNewPath(self, oldPath, newPath):
        ...
        for i in range(len(oldPath)):
            ...
            if distance(
                    oldFirstPoint,
                    newFirstPoint).kilometers > MAX_DISTANCE_BETWEEN_SAME_POINTS_KM:
                return True
